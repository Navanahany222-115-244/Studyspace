...
function statusBadgeClass(status) {
    ...
    if (status === 'BOOKED') return 'badge-orange';
    return 'bg-secondary';
}
...
// Formats a Date (or a date-ish value coming back from the API) as
// "YYYY-MM-DD" using LOCAL time - mirrors the backend's toLocalDateStr().
// IMPORTANT: never use toISOString().slice(0,10) for "today"/"this date"
// here - toISOString() converts to UTC, so in any timezone ahead of UTC
// (e.g. Dhaka, UTC+6) it silently returns the WRONG day for several hours
// around local midnight, which mis-dates bookings and makes a class that
// is actually running right now get excluded from "today's" bookings.
...
// Keeps the room currently open in the booking modal, so the
// submit handler can run a quick frontend capacity/overlap
// check before ever contacting the server.
...
function openBookModal(room) {
    ...
    document.getElementById('book_date').value = toLocalDateStr(new Date());
    document.getElementById('book_date').min = toLocalDateStr(new Date());
    ...
    document.getElementById('book_student_count').max = room.capacity;
...
document.getElementById('bookRoomForm').addEventListener('submit', async function (e) {
    ...
    const studentCount = Number(document.getElementById('book_student_count').value);

    if (start >= end) {
        showMessage('bookFormMessage', 'End time must be later than start time.', true);
        return;
    }

    // Frontend capacity check (backend re-checks this too - see note below)
    if (currentBookingRoom && studentCount > currentBookingRoom.capacity) {
        showMessage(
            'bookFormMessage',
            `Booking cannot be completed. This room can accommodate a maximum of ${currentBookingRoom.capacity} students.`,
            true
        );
        return;
    }

    // Frontend overlap check against the room's already-known next
    // booking, for instant feedback. This is a convenience check only -
    // the backend re-validates against the full database before ever
    // inserting, since frontend checks alone can always be bypassed.
    if (currentBookingRoom && currentBookingRoom.next_booking) {
        const nb = currentBookingRoom.next_booking;
        const nbDateStr = toLocalDateStr(nb.booking_date);
        if (nbDateStr === date && nb.start_time < end && nb.end_time > start) {
            showMessage('bookFormMessage', 'Room is already booked during this time.', true);
            return;
        }
    }
    ...
    try {
        ...
        bookings.forEach((b) => {
            ...
            row.innerHTML = `
                ...
                <td>${formatTime(b.end_time)}</td>
                <td>${b.student_count || ''}</td>
                <td><span class="badge badge-orange">Confirmed</span></td>
            `;
            tbody.appendChild(row);
        });
    } catch (error) {
...
// ---------------------------------------------------
// Auto-refresh every 30 seconds, so a room's status
// (e.g. BOOKED -> CLASS RUNNING -> AVAILABLE) updates
// on its own as time passes, with no manual reload.
// ---------------------------------------------------
setInterval(() => {
    loadRoomsWithStatus();
    loadTodaySchedule();
